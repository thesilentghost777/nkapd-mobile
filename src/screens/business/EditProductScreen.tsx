...
interface ImageItem {
  ...
  uploading: boolean;
  error: boolean;
  isExisting: boolean; // Pour différencier les images déjà uploadées
}
...
export default function EditProductScreen({ navigation, route }: EditProductScreenProps) {
  ...
  const loadProduct = async () => {
    try {
      const response = await api.getProduitDetails(productId);
      if (response.data.success) {
        const produit = response.data.data.produit;
        setTitre(produit.titre);
        setDescription(produit.description);
        setPrix(String(produit.prix));
        setCategorie(produit.categorie);
        setVille(produit.ville || '');
        setTelephoneContact(produit.telephone_contact || '');
        setWhatsapp(produit.whatsapp || '');
        
        // Convertir les URLs existantes en ImageItem
        const existingImages: ImageItem[] = (produit.images || []).map((url: string) => ({
          localUri: url,
          serverUrl: url,
          uploading: false,
          error: false,
  ...
  const retryUpload = async (index: number) => {
    ...
    if (!image || image.isExisting) return;
  ...
  const submitProduct = async () => {
    ...
    try {
      ...
      const response = await api.modifierProduit(productId, {
        titre: titre.trim(),
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.imagesContainer}>
            {images.map((item, index) => (
              <View key={index} style={styles.imageWrapper}>
                ...
                {/* Indicateur de succès (seulement pour les nouvelles images) */}
                {item.serverUrl && !item.uploading && !item.error && !item.isExisting && (
